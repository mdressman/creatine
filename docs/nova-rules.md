...
Built-in rules covering all three evaluation tiers:

| Rule | Type | Description |
|------|------|-------------|
| `PromptInjection` | Keyword | Classic injection phrases |
| `JailbreakDAN` | Keyword | DAN/Do-Anything-Now attacks |
| `JailbreakRoleplay` | Keyword | Persona/roleplay jailbreaks |
| `DataExfiltration` | Keyword | Data extraction attempts |
| `Obfuscation` | Keyword | Encoded/obfuscated attacks |
| `CommandInjection` | Keyword | System command injection |
| `HypotheticalJailbreak` | Keyword | Hypothetical scenario attacks |
| `SemanticJailbreakDetection` | Semantic | Paraphrased jailbreak attempts |
| `SemanticDataExfiltration` | Semantic | Varied data extraction attempts |
| `SemanticCommandExecution` | Semantic | Obfuscated command execution |
| `SemanticSQLInjection` | Semantic | Disguised SQL injection |
| `SemanticPrivilegeEscalation` | Semantic | Privilege escalation attempts |
| `LLMComprehensiveThreatAnalysis` | LLM | Deep analysis for all attack types |
| `KeywordShellCommands` | Keyword | Shell command patterns |
| `KeywordSQLInjection` | Keyword | SQL injection patterns |
| `KeywordCodeLanguages` | Keyword | Code execution in various languages |
| `KeywordPrivilegeKeywords` | Keyword | Privilege escalation keywords |
| `KeywordMultilingual` | Keyword | Attacks in Spanish, French, German, etc. |

### Community Rules (`rules/community/`)

Rules from the [Nova-Hunting/nova-rules](https://github.com/Nova-Hunting/nova-rules) repository, synced via `python creatine.py sync-rules`. Includes 50+ rules covering injection variants, jailbreak techniques, TTPs, policy puppetry, unicode attacks, APT-specific patterns, and more. Updated as the community contributes new rules.

### Generated Rules (`rules/feed_generated.nov`, `rules/learned_rules.nov`)
